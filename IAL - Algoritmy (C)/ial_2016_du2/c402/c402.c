...
**/
	...
	//az do konce, nebo nalezeni Cont.
	while (*RootPtr != NULL && (*RootPtr)->Cont != Content) {
		//vyhledam konec
		if ((*RootPtr)->Cont < Content) RootPtr = &((*RootPtr)->RPtr);
		else RootPtr = &((*RootPtr)->LPtr);
	}

	//pokud predchozi smycka nenasla shodu
	if (*RootPtr == NULL) { //vkladam list
...
**/
	...
	Leftmost_Preorder(RootPtr, &stack); // prvni vetev
	while (!SEmptyP(&stack)) {
...
**/
	...
	while (!SEmptyP(&stack)) {
		...
		if (STopPopB(&stackb)) { //jsem tu poprve
			SPushB(&stackb, FALSE);
			Leftmost_Postorder(RootPtr->RPtr, &stack, &stackb); // jdu vpravo
		}
		else { //navrat
			BTWorkOut(RootPtr);
			STopPopP(&stack); //smazu se
		}		
	}
}
