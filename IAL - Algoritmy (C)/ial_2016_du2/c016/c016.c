...
void htInit ( tHTable* ptrht ) {
	//memset(ptrht,0,sizeof(tHTItem)*HTSIZE); // vynulovani dat
...
tHTItem* htSearch(tHTable* ptrht, tKey key) {
	for (tHTItem *item = (*ptrht)[hashCode(key)]; item != NULL; item = item->ptrnext) {
...
void htInsert ( tHTable* ptrht, tKey key, tData data ) {
	...
	if (item != NULL) item->data = data; // polozka existuje v tabulce
	else {
...
void htDelete ( tHTable* ptrht, tKey key ) {
	...
	for (tHTItem *item = (*ptrht)[hashCode(key)]; item != NULL; item = item->ptrnext) {
		if (!strcmp(item->key, key)) {
			if (prev) prev->ptrnext = item->ptrnext;
			else (*ptrht)[hashCode(key)] = item->ptrnext;
			free(item);
			return;
		}
		prev = item;
